...
var hairLengths = window._mobaroHairLengths || [];
    ...
    function renderBookingStep()
    {
        ...
        if (currentBookingStep === 0) {
            ...
            if (selectedArtistId) {
                for (var i = 0; i < artists.length; i++) {
                    if (artists[i].id === selectedArtistId) {
                        artist = artists[i]; break; }
                }
            }
            var displayDate = '';
            if (selectedDate) {
    ...
    function showBookingConfirmation(data)
    {
        ...
        '<div class="text-center py-8">' +
            ...
            '<div class="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-6">' +
                ...
                '<a href="tel:' + (data.booking_phone || '03136662122').replace(/[^0-9]/g, '') + '" class="inline-block mt-3 bg-amber-500 text-white px-8 py-3 rounded-2xl font-bold text-lg hover:bg-amber-600 transition-colors">' +
