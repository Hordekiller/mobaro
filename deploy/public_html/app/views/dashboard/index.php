    ...
    <div class="max-w-[1400px] mx-auto px-8 py-8" style="padding: 30px auto;">
        <div class="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-8">
            ...
            <div class="bg-white rounded-[20px] p-6 shadow-[0_4px_30px_rgba(183,110,121,0.08)] h-fit lg:sticky lg:top-[100px]">
                ...
                <div id="phone-verify-banner" class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
                    <div class="flex items-start gap-3">
                        <i class="fa-solid fa-triangle-exclamation text-amber-500 mt-0.5"></i>
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-amber-800">شماره تلفن تأیید نشده</p>
                            <p class="text-xs text-amber-600 mt-1">برای استفاده کامل از امکانات، شماره خود را تأیید کنید.</p>
                            <button onclick="sendPhoneOtp()" id="send-otp-btn" class="mt-2 px-4 py-1.5 bg-amber-500 text-white text-xs font-semibold rounded-lg hover:bg-amber-600 transition-all">
                                <i class="fa-solid fa-paper-plane ml-1"></i>ارسال کد تأیید
                            </button>
                        </div>
                    </div>
                    <div id="otp-verify-section" class="hidden mt-3">
                        <div class="flex gap-2">
                            <input type="text" id="otp-code-input" maxlength="5" inputmode="numeric" placeholder="کد ۵ رقمی" class="flex-1 px-3 py-2 text-sm border border-amber-300 rounded-lg focus:border-amber-500 outline-none">
                            <button onclick="verifyPhoneOtp()" class="px-4 py-2 bg-emerald-500 text-white text-xs font-semibold rounded-lg hover:bg-emerald-600 transition-all">تأیید</button>
                        </div>
                    </div>
                </div>
                <?php endif; ?>
...
function sendPhoneOtp() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin ml-1"></i>در حال ارسال...';
    fetch('/dashboard/send-phone-otp', {
        method: 'POST',
        ...
        if (d.success) {
            ...
            btn.disabled = false;
            btn.innerHTML = '<i class="fa-solid fa-paper-plane ml-1"></i>ارسال کد تأیید';
        }
    }).catch(() => {
...
function verifyPhoneOtp() {
    ...
    if (!code) { showToast('کد تأیید را وارد کنید', 'error'); return; }
    fetch('/dashboard/verify-phone-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'code=' + encodeURIComponent(code) + '&' + csrfParam()
    }).then(r => r.json()).then(d => {
        if (d.success) {
            showToast(d.message, 'success');
