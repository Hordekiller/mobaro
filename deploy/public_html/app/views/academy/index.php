...
<?php if ($totalPages > 1) : ?>
<div class="max-w-screen-2xl mx-auto px-8 pb-12">
    <div class="flex justify-center items-center gap-2">
        <?php if ($page > 1) : ?>
        <a href="/academy?<?= http_build_query(array_merge($_GET, ['page' => $page - 1])) ?>" class="w-10 h-10 rounded-lg border border-zinc-200 text-zinc-500 hover:border-rose-400 hover:text-rose-500 transition flex items-center justify-center">
            <i class="fa-solid fa-chevron-right"></i>
        </a>
        <?php endif; ?>
        <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
        <a href="/academy?<?= http_build_query(array_merge($_GET, ['page' => $i])) ?>" class="w-10 h-10 rounded-lg flex items-center justify-center font-medium transition <?= $i === $page ? 'bg-rose-600 text-white' : 'border border-zinc-200 text-zinc-600 hover:border-rose-400 hover:text-rose-500' ?>"><?= $i ?></a>
        <?php endfor; ?>
