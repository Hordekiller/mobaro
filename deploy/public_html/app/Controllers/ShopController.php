...
class ShopController extends BaseController
{
    ...
    public function cart(): void
    {
        ...
        $this->view('home/cart', compact('cart', 'total', 'checkoutToken'));
    }
    ...
    public function checkout(): void
    {
        ...
        if (empty($checkoutToken) || ($checkoutToken !== ($_SESSION['checkout_token'] ?? ''))) {
            $this->json(['error' => 'توکن پرداخت نامعتبر است. لطفاً صفحه را رفرش کنید.'], 400);
            return;
        }
        unset($_SESSION['checkout_token']);

        $productItems = array_filter($cart, fn($item) => ($item['type'] ?? 'product') === 'product');
        if (!empty($productItems)) {
            $pids = array_column($productItems, 'id');
            $placeholders = implode(',', array_fill(0, count($pids), '?'));
            $stockRows = Database::fetchAll("SELECT id, stock, name FROM products WHERE id IN ({$placeholders})", $pids);
            $stockMap = array_column($stockRows, 'stock', 'id');
            foreach ($productItems as $item) {
                $avail = (int) ($stockMap[$item['id']] ?? 0);
                if ($avail < $item['qty']) {
                    $this->json(['error' => "موجودی «{$item['name']}» کافی نیست (موجودی: {$avail})."], 400);
                    return;
                }
            }
        }
        ...
        try {
            ...
            foreach ($cart as $item) {
                ...
                if (($item['type'] ?? 'product') === 'course') {
                    ...
                    Database::query("UPDATE products SET stock = stock - ? WHERE id = ? AND stock >= ?", [$item['qty'], $item['id'], $item['qty']]);
                }
            }
    ...
    public function paymentCallback(): void
    {
        ...
        Database::insert('payment_logs', [
            ...
            'request_data' => json_encode(['Amount' => $order['total'], 'Authority' => $authority]),
