...
class DashboardController extends BaseController
{
    ...
    public function cancelOrder(): void
    {
        ...
        $order = Database::fetch(
            "SELECT id, user_id, status, total, payment_method, payment_status, tracking_code FROM orders WHERE id = ? AND user_id = ?",
            [$id, Auth::id()]
        );
        if (!$order) {
        ...
        try {
            Database::update('orders', ['status' => 'cancelled', 'payment_status' => 'cancelled'], 'id = :id', ['id' => $id]);

            if ($order['payment_method'] === 'wallet' && $order['payment_status'] === 'paid') {
                $current = Database::fetch("SELECT wallet FROM users WHERE id = ?", [Auth::id()]);
                $newBalance = (int) ($current['wallet'] ?? 0) + (int) $order['total'];
                ...
                Database::insert('transactions', [
                    ...
                    'type' => 'wallet_deposit',
                    'amount' => $order['total'],
                    'description' => "بازپرداخت سفارش {$order['tracking_code']}",
                    'payment_status' => 'paid',
                ]);

                $_SESSION['user'] = Database::fetch("SELECT * FROM users WHERE id = ?", [Auth::id()]);
            }
    ...
    public function walletPaymentCallback(): void
    {
        ...
        Database::insert('payment_logs', [
            ...
            'request_data' => json_encode(['Amount' => $amount, 'Authority' => $authority]),
        ...
        if ($result['status']) {
            ...
            try {
                ...
                $current = Database::fetch("SELECT wallet FROM users WHERE id = ?", [Auth::id()]);
                $newBalance = (int) ($current['wallet'] ?? 0) + $amount;
                Database::update('users', ['wallet' => $newBalance], 'id = :id', ['id' => Auth::id()]);
    ...
    public function sendPhoneOtp(): void
    {
        ...
        $sms = new SmsService();
        if (!$sms->isConfigured()) {
            $this->json(['error' => 'سرویس پیامک فعال نیست.'], 400);
            return;
        }
        ...
        $phone = $user['phone'] ?? '';
        if (empty($phone)) {
            $this->json(['error' => 'شماره تلفن یافت نشد.'], 400);
            return;
        }

        RateLimiter::init();
        if (RateLimiter::isLocked('phone_otp_' . Auth::id())) {
            $this->json(['error' => 'لطفاً ۲ دقیقه صبر کنید.'], 429);
            return;
        }
        ...
        RateLimiter::recordAttempt('phone_otp_' . Auth::id(), false);

        $_SESSION['phone_verify_expires'] = time() + $ttl;

        if ($result['status']) {
            $this->json(['success' => true, 'message' => 'کد تأیید به شماره ' . $phone . ' ارسال شد.']);
        } else {
            $this->json(['error' => 'خطا در ارسال کد تأیید.'], 500);
        }
    }
    ...
    public function verifyPhoneOtp(): void
    {
        ...
        if (empty($code)) {
            $this->json(['error' => 'کد تأیید را وارد کنید.'], 400);
            return;
        }
        ...
        $row = Database::fetch(
            "SELECT id FROM verification_codes WHERE phone = ? AND code = ? AND purpose = 'phone_verify' AND used = 0 AND expires_at > NOW() ORDER BY id DESC LIMIT 1",
            [$phone, $code]
        );

        if (!$row) {
            $this->json(['error' => 'کد تأیید نادرست یا منقضی شده است.'], 400);
            return;
        }

        Database::update('verification_codes', ['used' => 1], 'id = :id', ['id' => $row['id']]);
        Database::update('users', ['phone_verified' => 1], 'id = :id', ['id' => Auth::id()]);

        unset($_SESSION['phone_verify_expires']);

        $_SESSION['user'] = Database::fetch("SELECT * FROM users WHERE id = ?", [Auth::id()]);

        $this->json(['success' => true, 'message' => 'شماره تلفن با موفقیت تأیید شد.']);
    }
}
