...
<div class="min-h-screen bg-gradient-to-br from-rose-50 via-white to-rose-50/30">
    <div class="relative bg-gradient-to-r from-rose-600 to-rose-800 py-20 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 text-center relative z-10">
            <span class="inline-block px-4 py-1 rounded-full bg-white/20 text-white text-sm font-medium mb-4">درباره <?= e($settings['brand_name'] ?? 'موبارو') ?></span>
            <h1 class="text-4xl md:text-5xl font-bold text-white mb-4"><?= e($settings['about_title'] ?? 'درباره ما') ?></h1>
            <p class="text-rose-100 text-lg max-w-2xl mx-auto"><?= e($settings['about_subtitle'] ?? 'سالن زیبایی حرفه‌ای با ' . ($settings['about_stat_years'] ?? '۱۲') . ' سال تجربه در ' . ($settings['brand_city'] ?? 'تهران')) ?></p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl p-8 md:p-12 mb-10">
            <div class="grid md:grid-cols-2 gap-10 items-start">
                <div>
                    ...
                    <img src="/assets/images/<?= e($aboutImage) ?>" alt="درباره <?= e($settings['brand_name'] ?? 'موبارو') ?>" class="w-full rounded-2xl object-cover shadow-lg">
                    <?php else : ?>
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-6">
                        <div class="bg-rose-50 rounded-xl p-4 text-center">
                            <span class="block text-2xl font-bold text-rose-600"><?= e($settings['about_stat_years'] ?? '۱۲') ?>+</span>
                            <span class="text-xs text-zinc-500">سال تجربه</span>
                        </div>
                        <div class="bg-rose-50 rounded-xl p-4 text-center">
                            <span class="block text-2xl font-bold text-rose-600"><?= e($settings['about_stat_clients'] ?? '۱۵۰۰') ?>+</span>
                            <span class="text-xs text-zinc-500">مشتری راضی</span>
                        </div>
                        <div class="bg-rose-50 rounded-xl p-4 text-center">
                            <span class="block text-2xl font-bold text-rose-600"><?= e($settings['about_stat_services'] ?? '۲۰') ?>+</span>
                ...
                <div class="prose prose-zinc max-w-none">
                    ...
                    <h3 class="text-xl font-bold text-zinc-800">چرا <?= e($settings['brand_name'] ?? 'موبارو') ?>؟</h3>
