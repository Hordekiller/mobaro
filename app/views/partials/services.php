...
<section id="services" class="py-24 bg-white">
    <div class="max-w-screen-2xl mx-auto px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div onclick="selectService(<?= (int) $service['id'] ?>)" role="button" tabindex="0" aria-label="انتخاب <?= e($service['title']) ?>" onkeydown="if(event.key==='Enter'||event.key===' ')selectService(<?= (int) $service['id'] ?>)" class="service-card bg-white border border-zinc-100 rounded-3xl overflow-hidden cursor-pointer">
