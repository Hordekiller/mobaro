...
<head>
    ...
    <title>ورود ادمین | <?= e($brandName) ?></title>
    ...
    <div class="w-full max-w-sm">
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <form method="POST" action="/admin/login" class="px-8 pb-8 space-y-5">
                ...
                <?php endif; ?>
                <div>
                    <label for="admin-username" class="text-xs text-zinc-500 block mb-2">نام کاربری</label>
                ...
                <div>
                    <label for="admin-password" class="text-xs text-zinc-500 block mb-2">رمز عبور</label>
                    <input id="admin-password" name="password" type="password" placeholder="••••••••"
                           class="w-full border rounded-3xl px-7 py-5 outline-none focus:border-rose-500 transition-colors">
                </div>
                <?php if ($captchaEnabled) : ?>
                <div>
                    <label for="admin-captcha" class="text-xs text-zinc-500 block mb-2">کد امنیتی</label>
                    ...
                    <input id="admin-captcha" name="captcha" type="text" inputmode="numeric" placeholder="پاسخ"
        ...
        <p class="text-center text-xs text-zinc-600 mt-6">© <?= date('Y') ?> <?= e($brandName) ?>. پنل مدیریت</p>
    </div>
    <script>
